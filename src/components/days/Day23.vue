...
const instructionLine = `Swap the first and last letters in the string`;
const functionHeader = `function swapLetters(str) {`;
const functionFooter = `}`;
const userFunctionBody = ref<string>(`// write your code here`);
...
function runUserFunction(str: string): string {
  try {
    ...
    testResult.value = `Code error: ${e?.message ?? "unknown error"}`;
    return "";
  }
}
...
function testFunction() {
  ...
  const t1 = normalize(runUserFunction("yerrm")) === "merry";
  const t2 = normalize(runUserFunction("aants")) === "santa";
  const t3 = normalize(runUserFunction("wnos")) === "snow";
...
<template>
  <div class="starry-bg">
    <div v-if="!showCode" class="puzzle-container">
      <div class="code-block" aria-label="Code editor">
      ...
      <button class="test-btn" type="button" @click="testFunction">
        {{ testResult === "Nope" ? "Nope" : `swapLetters("yerrm")` }}
      </button>
    </div>
    <div v-if="showCode" class="code">
      <h3>"Great"</h3>
      <h1>Merry Christmas to you too!</h1>
    </div>
  </div>
</template>
