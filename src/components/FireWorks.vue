...
function spawnRocket(x?: number) {
  ...
  rockets.push({
    ...
    y: height + 10,
    vx: rand(-0.6, 0.6),
    vy: rand(-14.5, -12.5), // Extra upward force
    targetY: rand(height * 0.04, height * 0.22), // Slightly higher target
    hue,
    alive: true,
  });
}
...
function tick(time: number) {
  ...
  // Fade the trail instead of clearing it completely
  ...
  for (const r of rockets) {
    ...
    r.x += r.vx * dt;
    r.y += r.vy * dt;
    r.vy += 0.18 * dt; // Apply a little gravity to the rocket

    // Draw the rocket as a small glowing point
    ctx.beginPath();
    ctx.fillStyle = `hsla(${r.hue}, 100%, 65%, 0.9)`;
    ctx.arc(r.x, r.y, 2.0, 0, Math.PI * 2);
    ctx.fill();

    // Explode when the rocket reaches its target or starts falling
  ...
  ctx.globalCompositeOperation = "lighter"; // Add light values for a pleasant glow
  ...
  // Keep the particle count manageable
...
function onPointerDown(e: PointerEvent) {
  // Click or tap to launch a rocket at that position
  const canvas = canvasRef.value;
  if (!canvas) return;
...
.fireworks {
  ...
  width: 100%;
  height: 100%;
  pointer-events: auto; /* Allow clicks and taps for extra fireworks */
}
</style>
